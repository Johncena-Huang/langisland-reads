...
const NotionPagePicker = () => {
  const [notionCollection, setNotionCollection] = useState([]);
  const [show, setShow] = useState(false);
  useEffect(() => {
    const handleMessageFromServer = (e) => {
      console.log("message received", e.origin);
      if (process.env.NODE_ENV !== "production") {
        if (e.origin !== process.env.REACT_APP_DOMAIN) return;
      } else {
        if (window.location.origin !== e.origin) return;
      }
      setNotionCollection(e.data);
      setShow(true);
    };
    window.addEventListener("message", handleMessageFromServer);
    return () => {
      window.removeEventListener("message", handleMessageFromServer);
    };
  });
  const renderNotionCollection = () => {
    if (!notionCollection) return;
    return notionCollection.map((page, index) => (
      <NotionCard key={index} page={page} closeModal={setShow} />
    ));
  };
  return (
    <Modal scrollable backdrop show={show} size="lg" centered>
      <Modal.Header closeButton onHide={() => setShow(false)}>
        ...
        <span className="align-text-bottom fs-2 ms-2">Notion</span>
      </Modal.Header>
      <Modal.Body className="row">{renderNotionCollection()}</Modal.Body>
    </Modal>
  );
};
